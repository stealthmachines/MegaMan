...
server {
    ...
    # Redirect root to admin page directly, avoiding loops
    location = / {
        return 302 /admin;
    }
}
...
# Configure NGINX stream module
echo "[+] Configuring NGINX stream module..."
cat > /etc/nginx/stream.conf <<EOL
stream {
    upstream pihole {
        server 127.0.0.1:80;
    }
...
# Include the stream configuration in nginx.conf
